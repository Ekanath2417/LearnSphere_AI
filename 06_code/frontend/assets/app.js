...
function taskRow(t) {
  return `<div class="task-row">
    ...
    <div class="row-grow"><b>${esc(t.title)}</b><small>${esc(t.subject_name || "General")} / ${esc(t.planned_minutes)} min / ${esc(t.due_date || "No date")}</small></div>
...
async function knowledge(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Saved notes</h2>${notes.map((n) => `<div class="note-row"><div class="row-grow"><b>${esc(n.title)}</b><small>${esc(n.subject || "General")} / Semester ${esc(n.semester || 1)} / ${esc(n.topic || "General")}</small></div></div>`).join("") || '<p class="empty">Your saved notes will appear here.</p>'}</section>
    </div>`;
...
async function syllabus(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Saved syllabus</h2>${records.map((x) => `<div class="note-row"><div class="row-grow"><b>${esc(x.subject_name)} / Unit ${esc(x.unit)}: ${esc(x.unit_title)}</b><small>Semester ${esc(x.semester)} / ${esc(x.topics)}</small></div></div>`).join("") || '<p class="empty">Add your first syllabus unit.</p>'}</section>
    </div>`;
...
async function pyq(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Question bank</h2>${records.map((x) => `<div class="note-row"><div class="row-grow"><b>${esc(x.subject)} / ${esc(x.exam_year)}</b><small>${esc(x.exam_type)} / ${esc(x.question || "PDF question paper attached")}</small></div></div>`).join("") || '<p class="empty">Add a previous-year question to begin.</p>'}</section>
    </div>`;
...
async function timetable(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Weekly timetable</h2>${records.map((x) => `<div class="note-row"><div class="row-grow"><b>${esc(x.day)} / ${esc(x.subject)}</b><small>${esc(x.start_time)}-${esc(x.end_time)} / ${esc(x.room || "Room TBA")}</small></div></div>`).join("") || '<p class="empty">Add your first timetable entry.</p>'}</section>
    </div>`;
...
async function practice(view) {
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Practice principle</h2><p class="empty">Short checks work best after a focused revision block. The current MVP generates starter questions locally and keeps students in control.</p></section>
    </div>`;
  $("#quiz-form").onsubmit = async (event) => {
    ...
    $("#answer-form").onsubmit = async (answerEvent) => {
      ...
      answerEvent.currentTarget.querySelector("button").disabled = true;
    };
  };
}
...
async function insights(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      <section class="card"><h2>Consistency signal</h2><div class="insight-score">${d.consistency}<small>%</small></div><p class="empty">Recorded focus time relative to planned workload.</p></section>
      <section class="card"><h2>Indicative mark scenario</h2><div class="insight-score">${d.predicted_mark}<small>%</small></div><p class="empty">${esc(d.disclaimer)}</p></section>
    </div>
...
async function diary(view) {
  ...
  view.innerHTML = `
    ...
    <div class="split">
      ...
      <section class="card"><h2>Recent entries</h2>${entries.map((x) => `<div class="note-row"><div class="row-grow"><b>${esc(x.entry_date)} / ${esc(x.mood)}</b><small>${esc(x.body)}</small></div></div>`).join("") || '<p class="empty">Your reflections will become a useful learning record.</p>'}</section>
    </div>`;
